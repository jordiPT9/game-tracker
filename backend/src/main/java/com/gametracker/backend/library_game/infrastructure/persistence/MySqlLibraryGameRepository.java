...
public class MySqlLibraryGameRepository implements LibraryGameRepository {
    ...
    public MySqlLibraryGameRepository(
            LibraryGameJpaRepository libraryGameJpaRepository, LibraryGameMapper libraryGameMapper) {
    ...
    public LibraryGame findByTitleAndUsername(String title, String username) {
        LibraryGameJpaEntity libraryGameJpaEntity =
                libraryGameJpaRepository.findByTitleAndUser_Username(title, username);
    ...
    public List<LibraryGame> findByUsername(String username) {
        List<LibraryGameJpaEntity> libraryGameJpaEntities =
                libraryGameJpaRepository.findByUser_Username(username);

        return libraryGameJpaEntities.stream().map(libraryGameMapper::mapToDomainEntity).toList();
    }

    @Override
    public Double getAverageGameRating(String gameTitle) {
        return libraryGameJpaRepository.getAverageGameRating(gameTitle);
    }
